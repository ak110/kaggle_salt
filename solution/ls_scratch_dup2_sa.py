...
def _create_network():
    ...
    def spatial_attention(filters):
        def layers(x):
            a = builder.conv2d(1, 3, activation='sigmoid')(x)
            v = builder.conv2d(filters, 1, use_act=False)(x)
            x = keras.layers.add([x, keras.layers.multiply([a, v])])
            return x
        return layers

    x = tk.dl.layers.coord_channel_2d()(x_channel=False)(x)
    x = builder.conv2d(64, 3, use_act=False)(x)
    x = builder.res_block(64)(x)
    x = builder.res_block(64)(x)
    x = builder.res_block(64)(x)
    x = builder.res_block(64)(x)
    x = builder.bn_act()(x)
    x = spatial_attention(64)(x)
    x = tk.dl.layers.coord_channel_2d()(x_channel=False)(x)
    x = builder.conv2d(128, 3, strides=2, use_act=False)(x)  # 56
    x = builder.res_block(128)(x)
    x = builder.res_block(128)(x)
    x = builder.res_block(128)(x)
    x = builder.res_block(128)(x)
    x = builder.bn_act()(x)
    x = spatial_attention(128)(x)
    x = tk.dl.layers.coord_channel_2d()(x_channel=False)(x)
    x = builder.conv2d(256, 3, strides=2, use_act=False)(x)  # 28
    x = builder.res_block(256)(x)
    x = builder.res_block(256)(x)
    x = builder.res_block(256)(x)
    x = builder.res_block(256)(x)
    x = builder.bn_act()(x)
    x = spatial_attention(256)(x)
    x = tk.dl.layers.coord_channel_2d()(x_channel=False)(x)
    d = x
    x = builder.conv2d(512, 3, strides=2, use_act=False)(x)  # 14
    x = builder.res_block(512)(x)
    x = builder.res_block(512)(x)
    x = builder.res_block(512)(x)
    x = builder.res_block(512)(x)
    x = builder.bn_act()(x)
    x = spatial_attention(512)(x)
    x = tk.dl.layers.coord_channel_2d()(x_channel=False)(x)
    x = builder.conv2d(512, 3, strides=2, use_act=False)(x)  # 7
    x = builder.res_block(512)(x)
    x = builder.res_block(512)(x)
    x = builder.res_block(512)(x)
    x = builder.res_block(512)(x)
    ...
    d = builder.conv2d(256, use_act=False)(d)

...
def _train_impl(args):
    ...
    x = inputs[0]
    x = pre_input = builder.preprocess()(x)
    ...
    for stage, (d, filters) in list(enumerate(zip(down_list, [32, 64, 128, 256, 512])))[::-1]:
        if stage == len(down_list) - 1:
            ...
            if stage in (2,):
                x = tk.dl.layers.pad2d()(((0, 1), (0, 1)), mode='reflect')(x)
            elif stage in (1,):
                x = tk.dl.layers.pad2d()(((1, 1), (1, 1)), mode='reflect')(x)
            elif stage in (0,):
                x = tk.dl.layers.pad2d()(((2, 3), (2, 3)), mode='reflect')(x)
        ...
        x = builder.res_block(filters, dropout=0.25)(x)
        x = builder.bn_act()(x)
    p = tk.dl.layers.resize2d()((125, 125))(pre_input)

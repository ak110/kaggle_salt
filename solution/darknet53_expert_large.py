...
def _train(args, fine=False):
    ...
    if True:
        ...
        salt_test_pi = np.uint8(pred_test[pi] > 0.5).mean(axis=(1, 2, 3))
        mask_test = np.logical_and(salt_test_pi > 0, salt_test_pi < 0.25)
        logger.info(f'pseudo label size: {len(X_test[pi][mask_test])}')
        gen.add(tk.generator.RandomPickData(X_test[pi][mask_test], pred_test[pi][mask_test]))
